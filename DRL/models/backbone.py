...
class TransporterNets(nn.Module):
  ...
  def setup(self):
    ...
    self.crop_size = 64
    self.crop_conv = nn.Conv(features=1, kernel_size=(self.crop_size, self.crop_size),
                             use_bias=False, dtype=jnp.float32, padding='SAME')

  def __call__(self, x, text, p=None, train=True): # x is observation as the image, text is encoded text by CLIP
    B, H, W, C = x.shape
